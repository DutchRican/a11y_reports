import { Link, useLocation } from 'react-router-dom';
import { useProjectContext } from '../context/projectContext';

interface NavigationProps {
  onUploadClick: () => void;
  onProjectCreationClick: () => void;
}

export default function Navigation({ onUploadClick, onProjectCreationClick }: NavigationProps) {
  const location = useLocation();
  const { projectID } = useProjectContext();

  const isActive = (path: string) => location.pathname === path;

  return (
    <nav className="bg-indigo-900 dark:bg-gray-900 text-white p-4 flex justify-between items-center sticky top-0 z-50">
      <div className="flex space-x-4">
        <Link to="/" className={`px-3 py-2 rounded-md text-sm font-medium ${isActive('/') ? 'bg-indigo-700 dark:bg-gray-700' : 'hover:bg-indigo-700 dark:hover:bg-gray-700'}`}>
          Home
        </Link>
        {projectID ? (
          <>
            <Link to={`/project/${projectID}`} aria-disabled={!projectID} className={`px-3 py-2 rounded-md text-sm font-medium ${isActive(`/project/${projectID}`) ? 'bg-indigo-700 dark:bg-gray-700' : 'hover:bg-indigo-700 dark:hover:bg-gray-700'}`}>
              Overview
            </Link>
            <Link to={`project/${projectID}/reports`} aria-disabled={!projectID} className={`px-3 py-2 rounded-md text-sm font-medium ${isActive(`/project/${projectID}/reports`) ? 'bg-indigo-700 dark:bg-gray-700' : 'hover:bg-indigo-700 dark:hover:bg-gray-700'}`}>
              Reports
            </Link>
          </>
        ) : (
          <span className={`px-3 py-2 rounded-md text-sm font-medium text-gray-400`}>
            Overview
          </span>
        )}
        <Link to="/ada-info" className={`px-3 py-2 rounded-md text-sm font-medium ${isActive('/ada-info') ? 'bg-indigo-700 dark:bg-gray-700' : 'hover:bg-indigo-700 dark:hover:bg-gray-700'}`}>
          ADA Info
        </Link>
      </div>
      {isActive('/') && (
        <button
          onClick={onProjectCreationClick}
          className="ml-auto px-4 py-2 rounded-md text-sm font-medium bg-indigo-700 dark:bg-gray-700 hover:bg-indigo-600 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50"
        >Create Project</button>
      )}
      {projectID && !isActive('/') && (
        <button
          onClick={onUploadClick}
          className="ml-auto px-4 py-2 rounded-md text-sm font-medium bg-indigo-700 dark:bg-gray-700 hover:bg-indigo-600 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50"
        >
          Upload Scan
        </button>)}
    </nav>
  );
}
